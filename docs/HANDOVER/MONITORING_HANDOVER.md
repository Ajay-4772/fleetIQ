# FleetIQ — Observability, Metrics & Monitoring Handover
...
FleetIQ is observable across the entire request and telemetry lifecycle:
...
groups:
  - name: fleetiq-production-alerts
    rules:
      - alert: FleetIQHighErrorRate
        expr: rate(http_server_requests_seconds_count{status=~"5.."}[5m]) / rate(http_server_requests_seconds_count[5m]) > 0.02
        for: 2m
        labels:
          severity: critical
        annotations:
          summary: "FleetIQ HTTP 5xx error rate exceeds 2% over 5 minutes."

      - alert: FleetIQDatabaseConnectionPoolSaturation
