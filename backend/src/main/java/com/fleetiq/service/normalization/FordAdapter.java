...
@Component
public class FordAdapter implements OemAdapter {

    @Override
    public boolean supports(String source, Map<String, Object> payload) {
        if ("SIMULATED_FORD".equalsIgnoreCase(source)) {
            return true;
        }
        return payload.containsKey("vehicleIdentifier") && (payload.containsKey("oilLifePercentage") || payload.containsKey("idleDuration"));
    }
    ...
    public CanonicalVehicleEvent normalize(String source, Map<String, Object> payload, String rawPayload) {
        ...
        if (vIdObj == null || vIdObj.toString().trim().isEmpty()) {
            throw new IllegalArgumentException("Ford payload missing valid vehicleIdentifier");
        }
        String vehicleId = vIdObj.toString().trim();

        Double oilLife = getDouble(payload.get("oilLifePercentage"));
        if (oilLife != null && (oilLife < 0 || oilLife > 100)) {
            throw new IllegalArgumentException("Ford payload oilLifePercentage out of valid range 0-100: " + oilLife);
        }

        Integer idleMinutes = getInteger(payload.get("idleDuration"));
        if (idleMinutes != null && idleMinutes < 0) {
            throw new IllegalArgumentException("Ford payload idleDuration cannot be negative: " + idleMinutes);
        }

        String fault = payload.get("diagnosticCode") != null ? payload.get("diagnosticCode").toString().trim() : null;
        if (fault != null && (fault.isEmpty() || "null".equalsIgnoreCase(fault))) {
            fault = null;
        }

        Double batteryPct = getDouble(payload.get("batteryState"));
        Long odometer = getLong(payload.get("mileage"));
